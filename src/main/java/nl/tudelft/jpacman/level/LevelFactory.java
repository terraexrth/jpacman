...
public class LevelFactory {
	 ...
	 */
	Ghost createGhost(int level) {
		ghostIndex++;
		ghostIndex %= GHOSTS;
		switch (ghostIndex) {
			case BLINKY:
				return ghostFact.createBlinky(level);
			case INKY:
				return ghostFact.createInky(level);
			case PINKY:
				return ghostFact.createPinky(level);
			case CLYDE:
				return ghostFact.createClyde(level);
			default:
				return new RandomGhost(sprites.getGhostSprite(GhostColor.RED, level));
		}
	}
	 ...
	 */
	public Pellet createPellet(int level) {
		return new Pellet(PELLET_VALUE, sprites.getPelletSprite(level));
	}
